...
Alternatively set `OPENAI_API_KEY` / `OPENAI_ADMIN_KEY` in `.env`. The model is chosen in the Settings UI (`OPENAI_MODEL` is just the initial default). Restart the API (and worker if needed) after adding a key.
